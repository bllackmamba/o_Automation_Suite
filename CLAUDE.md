...
| 6 | **Manual-mode stage-by-stage matching (SC Available: NO)** — researcher sees `count_dist` for stage i BEFORE entering SC for stage i; engine pauses, then completes stage i's split and shows stage i+1's distribution. **Decided approach: Option A (decide-as-you-go).** | Phase 1–2b DONE / Phase 3 NOT STARTED |
...
**Phase 3 — NOT STARTED:** Container Dashboards UI — "SC Available: NO" path wires to `run_matching_step` instead of `run_matching`. Per stage: display `count_dist` (the `{"S0": n, "S1": n, ...}` dict returned in the paused result) so the researcher can choose which count values to select; take that input as `sc_for_stage`; call `run_matching_step(resume_state, sc_for_stage=<chosen list>)` on "Continue". When `paused=False`, render `fig9_table`/`breakdown`/`selected`/`unselected` exactly as the existing "SC Available: YES" path does today. |
