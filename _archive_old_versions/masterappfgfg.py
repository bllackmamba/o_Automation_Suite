...
def _auto_load_b(game_key: str = "sat") -> pd.DataFrame:
    ...
    for path in candidates:
        try:
            ...
            # ── FIXED: store ACTUAL column positions alongside names ──────────
            # Using enumerate(w_cols) was wrong — it used the 0-based list index
            # as the column position, shifting data by however many non-w columns
            # precede the first w-header.  Now we track the real column index.
            w_col_pairs = [
    ...
    for t in tables:
        ...
        for _, row in t.iterrows():
            try:
                ...
                if not (1 <= n <= pool):
                    continue
                def _get(key, _cols_lower=cols_lower, _row=row):
                    c = _find_col(_cols_lower, COL_MAP[key])
                    if c is None:
                        return None
                    try:
                        return int(float(str(_row[c]).strip()))
                    except Exception:
                        return None
                rows.append({
                    "number":     n,
...
def fetch_draw_history(game_key: str, pages: int = 3):
    ...
    if not freq_url:
        return None
    hist_base = freq_url.replace("/number-frequencies/", "/history/")
    all_rows = []
    try:
        import urllib.request
        for pg in range(1, pages + 1):
            ...
            for t in tables:
                cols_lower = {str(c).strip().lower(): c for c in t.columns}
                draw_c = next((cols_lower[k] for k in cols_lower
                               if k in ("draw", "draw no", "draw no.", "draw number")), None)
                num_c  = next((cols_lower[k] for k in cols_lower
                               if "winning" in k or k == "numbers"), None)
                date_c = next((cols_lower[k] for k in cols_lower
                               if "date" in k), None)
                if not (draw_c and num_c):
                    continue
                for _, row in t.iterrows():
                    try:
                        draw_no = int(float(str(row[draw_c]).strip()))
                        raw_nums = str(row[num_c]).strip()
                        nums = sorted([int(x) for x in re.split(r"[,\s]+", raw_nums)
                                       if x.strip().isdigit()])
                        date_str = str(row[date_c]).strip() if date_c else ""
                        pb_c = next((cols_lower[k] for k in cols_lower
                                     if "powerball" in k or k == "pb"), None)
                        pb = None
                        if pb_c:
                            try:
                                ...
                                pass
                        all_rows.append({"draw": draw_no, "date": date_str,
...
def append_draw_to_b(b_df: pd.DataFrame, numbers: list,
    ...
    """Append a new draw (sorted list of ints) to a column-oriented B DataFrame.

    Positional mapping: w1 = smallest number, w2 = second-smallest, etc.
    ...
    if b_df is None or b_df.empty:
        data = {f"w{i+1}": pd.Series([n], dtype="Int64") for i, n in enumerate(nums)}
        ...
        if b_rules_candidates:
            ...
            try:
                ...
                if sheet is not None:
                    ...
                    # FIX: store (actual_col_index, header_name) so we read data
                    # from the CORRECT column — previously used enumerate index
                    # which was wrong whenever non-w columns preceded the w-headers.
                    w_col_pairs_s1 = [
...
with vtabs[7]:
    ...
    if _do_fetch_freq and _st_freq_url:
        ...
        if _freq_df is not None and not _freq_df.empty:
            ...
            st.error("❌ Could not parse frequency table. Check URL or try again later.")
    ...
    if not _freq_view.empty:
        st.caption(f"{len(_freq_view)} numbers | "
                   f"last fetched: {_st_freq_path.stat().st_mtime:.0f}"
                   if _st_freq_path.exists() else "")
        ...
        # Quick Since-Last digest
        if "since_last" in _freq_view.columns and "number" in _freq_view.columns:
            sl_from_freq = dict(zip(_freq_view["number"].astype(int),
                                    _freq_view["since_last"].astype(int)))
    ...
    if not _b_for_stats.empty:
        _wcols_b = [c for c in _b_for_stats.columns if str(c).lower().startswith("w")]
