    ...
    try:
        ...
        n_cols = sorted(
            [c for c in main_df.columns
             if re.match(r'^n\d+$', c, re.I) or
             pd.to_numeric(main_df[c], errors="coerce").notna().mean() > 0.9],
...
def _save_df(df: pd.DataFrame, path: Path):
    ...
    if df.empty:
        return
    nc = sorted([c for c in df.columns if re.match(r'^n\d+$', c)],
...
def _count_matches(df: pd.DataFrame, n_cols: list,
    ...
    for start in range(0, len(df), CHUNK_SIZE):
        ...
        arr    = chunk[n_cols].to_numpy(dtype=np.float32)
        arr    = np.nan_to_num(arr, nan=-1.0)
        all_counts = np.concatenate([all_counts, _match_chunk(arr, cvi_nums)])
    return all_counts
...
def run_matching(main_df: pd.DataFrame,
    ...
    for w in w_cols:
        ...
        if raw:
            mc = _count_matches(main_df, n_cols, np.array(raw, dtype=np.float32))
    ...
    for idx, w in enumerate(w_cols):
        ...
        pres_counts = _count_matches(present_df, n_cols, cvi_nums)
...
def execute_collation(components: list[str]) -> pd.DataFrame:
    """
    For each component: drop col-0 (index), concatenate data columns,
    relabel w1, w2, w3… Typed ints throughout.
    """
    pieces = []
...
def build_w_matrix(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    n_cols = sorted([c for c in df_raw.columns if re.match(r'^n\d+$',c)],
                    key=lambda x: int(x[1:]))
    if not n_cols:
        return pd.DataFrame()
    rows_as_lists = []
    for _, row in df_raw.iterrows():
        nums = [int(row[c]) for c in n_cols
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if not raw_files and not global_files:
        ...
        if st.button("🔄 BUILD W-MATRIX & SLICE Ep / Sp / So",
            ...
            if w_mat.empty:
                st.error("No n-columns found. Ensure D file has n1…nN columns.")
            else:
        ...
        if up_sl:
            try:
                ...
                if sl_col and num_col:
                    ...
                    to_keep_up = [all_wt_up[rank]
                                  for rank in range(len(all_wt_up))]
                    save_data = {
